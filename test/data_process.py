...
for i in xrange(0,num_files):
        ...
        if file_labels[i] != TESTING_LABEL:
                ...
                if file_labels[i] == TRAINING_LABEL:
                        ...
                        if train_data==[]:
                                train_data = np.vstack((sp_location.T,sp_color.T, sp_size.T,sp_HOG.T,sp_colorH.T)).T
                        else:
                                temp = np.vstack((sp_location.T,sp_color.T, sp_size.T, sp_HOG.T,sp_colorH.T)).T
                                train_data = np.vstack((train_data,temp))
                else:
                        # get superpixel valid files
                        superpixels.append(im_sp)
                        valid_files = sp.getSuperValidFiles(im_sp, valid_files_count, valid_files)
                        valid_pixels_labels.append(sp.getPixelLabel(label_image))
                        ...
                        if valid_data==[]:
                                valid_data = np.vstack((sp_location.T,sp_color.T, sp_size.T,sp_HOG.T,sp_colorH.T)).T
                        else:
                                temp = np.vstack((sp_location.T,sp_color.T, sp_size.T, sp_HOG.T,sp_colorH.T)).T
