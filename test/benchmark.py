...
def accuracyOfSuperpixels(x,valid_files, valid_data, clf, valid_labels):
    count_correct=0
...
def accuracyOfPixels(x,valid_files, superpixels, valid_data, clf, valid_pixels_labels):
    count_correct = 0
    total_count = 0
    indicess = []
    for i in range(0, valid_files.shape[0]):
    ...
    for i in range(0,total_samples):
        temp = np.array(superpixels[x][0])
 #       index = []
        temp2 = np.array(valid_pixels_labels[x][0])
